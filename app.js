...
// create constants for the form and the form controls and display section
...
function checkDatesInvalid(startDate, endDate) {
	...
	if (!startDate || !endDate || startDate > endDate) {
		// TODOs to make the validation robust we could:
...
function getAllStoredPeriods() {
	...
	// If no periods were stored, default to an empty array; else, return the stored data as parsed JSON
...
function renderPastPeriods() {
	...
	// @todo: refactor to display an empty list
	if (periods.length === 0) {
		return;	
	}
